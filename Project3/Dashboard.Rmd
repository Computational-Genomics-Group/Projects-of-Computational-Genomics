...
sparsity = round(tot_zeros/length(FEATURE_TABLE)*100, digits = 1)
```
## OTU counts
...
my.alr <- function (data){
  ...
  # select reference (lowest number of zeroes)
...
# TODO remove comment if you want to compute imputation 
...
In particular will be produced three matrices but we consider only the one which preserve the original lib size
```{r}
#ft.mbimpute.origlibsize=t(ft.mbimputed$imp_count_mat_origlibsize)

ft.mbimpute.origlibsize=read.csv("mbimpute_origls.csv")
rownames(ft.mbimpute.origlibsize)=ft.mbimpute.origlibsize[,1]
ft.mbimpute.origlibsize=ft.mbimpute.origlibsize[,-1]
...
Code provided in the github repository `ruochenj/mbImpute` (from the original authors) presents a bug:
...
CSS with clr transformed data does not work well since it produces many NaN values, even where not originally present in the clr transformed data. So we decided to not use it
```{r}
...
findDA <- function(data){
  # find biggest q-value q' smaller than 0.05
  # all q-value smaller than q' are considered DA
  qv=data$wi.eBH
...
DA.imp.gmpr.aldex <- findDA(aldex.imp.gmpr)
DA.imp.css.aldex<- findDA(aldex.imp.css)
```


```{r, include=FALSE}
s1=sum(DA.imp.gmpr.aldex)
s2=sum(DA.imp.css.aldex)
```
Number of DA taxa from Aldex using:
* imputed, GMPR normalized data is `r s1`
* imputed, CSS normalized data is `r s2`
...
js1=JacSim(DA.ancom, DA.imp.gmpr.ancom)
js2=JacSim(DA.ancom,DA.gmpr.ancom)
js3=JacSim(DA.imp.css.aldex,DA.imp.gmpr.aldex)
js4=JacSim(DA.imp.gmpr.aldex,DA.imp.gmpr.ancom)
```
Jaccard similarity between DA taxa are:
* `r js1` in ancom, between Raw data and imputer-GMPR data
* `r js2` in ancom, between Raw data and GMPR normalized data
* `r js3` in aldex, between imputed-css and imputed-GMPR data
* `r js4` using imputed and GMPR normalized data, between aldex and ancom results
